...
class Blackboard:
    ...
    def draw_gross_x(self, start_row, start_col):
        for j in range(2):
            for i in range(2):
                self.write_horizontally(
                    "#", start_row + j * 4, start_col + i * 2)
...
try:
    with open("dane.csv", "r") as f:
        ...
        for i, round_answers in enumerate(answers):
            answers[i] = sorted(
                round_answers, key=lambda x: int(x[1]), reverse=True)
except FileNotFoundError:
    if messagebox.showerror(
        "ERROR", "There is no file named 'dane.csv' in the current directory"
    ):
        sys.exit()
...
button = tkinter.Button(
    window1, text="Go", command=lambda: game1.draw_gross_x(3, 2))
label.pack()
inputUser.pack()
labelPassword.pack()
inputPassword.pack()
...
while running:
    ...
    if surface.get_width() < surface.get_height() * (192 / 108):
        ...
        block_x = 0
        block_y = (surface.get_height() -
                   (block_height * 10) - (9 * 2) - 100) / 2
    else:
        block_height = (surface.get_height() - 100 - (9 * 2)) / 10
        block_width = block_height * 2 / 3

        # Move blocks to the center of the screen
        block_x = (surface.get_width() -
                   (block_width * 29) - (28 * 2) - 125) / 2
        block_y = 0
    ...
    pygame.draw.rect(
        ...
        (
            game1.stroke,
            game1.stroke,
            surface.get_width() - game1.stroke * 2,
            surface.get_height() - game1.stroke * 2,
        ),
    )
    ...
    for i in range(10):
        for j in range(29):
            ...
            pygame.draw.rect(
                surface, (0, 0, 0), (pos_x, pos_y, block_width, block_height)
            )
            surface.blit(
                label,
                (
                    pos_x + block_width * 0.146,
                    pos_y + block_height / 2 - letter_hight / 2,
                ),
            )
